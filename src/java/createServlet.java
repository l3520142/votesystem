...
public class createServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        int sqlpc=0;
        int pincode=(int)(Math.random()*100000); 
        ResultSet rs=sqlCreateVote.Querypincode(pincode);
        try{
        while(rs.next()){
        sqlpc=rs.getInt("pincode");}}catch(Exception e){}
        while(pincode==sqlpc)
        {pincode=(int)(Math.random()*100000);}
                
        sqlCreateVote sc=new sqlCreateVote(pincode,votename,0,0,0,0,optionA,optionB,optionC,optionD,endtime);
            sc.add();
        session.setAttribute("SC", sc);
        response.sendRedirect("showcreateresult.jsp");
    }
}
